...
select_species <- function(comb.df, sp.name) {
  ...
  d.select$pres[is.na(d.select$pres)] <- 0 # replace any NAs here with (observed) zeros
  d.select
}

# takes comb.df and species name and returns result of simple occupancy model
fit_species <- function(sp.name) {
  cat("Fitting species ", sp.name)
  d.select <- select_species(comb.df, sp.name = sp.name)
  ...
  # a named list of dataframes each RxJ
  canopy <- d.select |>
